...
def pipeline( img_src ):
    ...
    data = convolve(data, 6, 'gaussian')

    show_image(data, 'before')

    data = extract_lines(data, 125, 10, 90, 210, 7)
...
def extract_lines( image, min_line_length=10, max_line_gap=10, canny_thresh1=50, canny_thresh2=150, apertureSize=7 ):
    print('Extracting lines')
    image_copy = copy.deepcopy(image)
    image = np.uint8(image)
    if image.ndim > 2:
        image = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(image,canny_thresh1,canny_thresh2,apertureSize )
...
result = pipeline('STREAK_test_1-026.fit')
# show_image(gauss_kernel(10), 'kernel')
